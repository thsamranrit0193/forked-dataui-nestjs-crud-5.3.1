...
class CrudRoutesFactory {
    ...
    mergeOptions() {
        ...
        this.options.auth = crud_util_1.isObjectFull(authOptions) ? authOptions : {};
        if (crud_util_1.isUndefined(this.options.auth.property)) {
            this.options.auth.property = module_1.CrudConfigService.config.auth.property;
        }
        if (crud_util_1.isUndefined(this.options.auth.groups)) {
            this.options.auth.groups = module_1.CrudConfigService.config.auth.groups;
        }
        if (crud_util_1.isUndefined(this.options.auth.classTransformOptions)) {
            this.options.auth.classTransformOptions =
                module_1.CrudConfigService.config.auth.classTransformOptions;
        }
        const query = crud_util_1.isObjectFull(this.options.query) ? this.options.query : {};
        this.options.query = { ...module_1.CrudConfigService.config.query, ...query };
        const routes = crud_util_1.isObjectFull(this.options.routes) ? this.options.routes : {};
        this.options.routes = deepmerge(module_1.CrudConfigService.config.routes, routes, {
            arrayMerge: (a, b, c) => b,
        });
        const operators = crud_util_1.isObjectFull(this.options.operators) ? this.options.operators : {};
        this.options.operators = deepmerge(module_1.CrudConfigService.config.operators, operators);
        this.options.params = crud_util_1.isObjectFull(this.options.params)
            ? this.options.params
            : crud_util_1.isObjectFull(module_1.CrudConfigService.config.params)
        ...
        if (!crud_util_1.isObjectFull(this.options.dto)) {
            this.options.dto = {};
        }
        const serialize = crud_util_1.isObjectFull(this.options.serialize) ? this.options.serialize : {};
        this.options.serialize = { ...module_1.CrudConfigService.config.serialize, ...serialize };
        this.options.serialize.get = crud_util_1.isFalse(this.options.serialize.get)
            ? false
            : this.options.serialize.get || this.modelType;
        this.options.serialize.getMany = crud_util_1.isFalse(this.options.serialize.getMany)
            ? false
            : this.options.serialize.getMany
                ? this.options.serialize.getMany
                : crud_util_1.isFalse(this.options.serialize.get)
                    ? false
                    : serialize_helper_1.SerializeHelper.createGetManyDto(this.options.serialize.get, this.modelName);
        this.options.serialize.create = crud_util_1.isFalse(this.options.serialize.create)
            ? false
            : this.options.serialize.create || this.modelType;
        this.options.serialize.update = crud_util_1.isFalse(this.options.serialize.update)
            ? false
            : this.options.serialize.update || this.modelType;
        this.options.serialize.replace = crud_util_1.isFalse(this.options.serialize.replace)
            ? false
            : this.options.serialize.replace || this.modelType;
        this.options.serialize.delete =
            crud_util_1.isFalse(this.options.serialize.delete) ||
    ...
    canCreateRoute(name) {
        ...
        if (crud_util_1.isArrayFull(only)) {
            return only.some((route) => route === name);
        }
        if (crud_util_1.isArrayFull(exclude)) {
            return !exclude.some((route) => route === name);
        }
        return true;
    }
    setResponseModels() {
        const modelType = crud_util_1.isFunction(this.modelType)
            ? this.modelType
            : serialize_helper_1.SerializeHelper.createGetOneResponseDto(this.modelName);
        this.swaggerModels.get = crud_util_1.isFunction(this.options.serialize.get)
        ...
        this.swaggerModels.create = crud_util_1.isFunction(this.options.serialize.create)
            ? this.options.serialize.create
            : modelType;
        this.swaggerModels.update = crud_util_1.isFunction(this.options.serialize.update)
            ? this.options.serialize.update
            : modelType;
        this.swaggerModels.replace = crud_util_1.isFunction(this.options.serialize.replace)
            ? this.options.serialize.replace
            : modelType;
        this.swaggerModels.delete = crud_util_1.isFunction(this.options.serialize.delete)
            ? this.options.serialize.delete
            : modelType;
        this.swaggerModels.recover = crud_util_1.isFunction(this.options.serialize.recover)
    ...
    overrideRoutes(routesSchema) {
        crud_util_1.getOwnPropNames(this.targetProto).forEach((name) => {
            const override = reflection_helper_1.R.getOverrideRoute(this.targetProto[name]);
            const route = routesSchema.find((r) => crud_util_1.isEqual(r.name, override));
    ...
    overrideParsedBodyDecorator(override, name) {
        ...
        const withBody = crud_util_1.isIn(override, allowed);
        ...
        if (withBody && parsedBody) {
            ...
            if (crud_util_1.isEqual(override, 'createManyBase')) {
                ...
                const toCopy = crud_util_1.isIn(metatype, types) || crud_util_1.isNil(metatype);
                if (toCopy) {
    ...
    getPrimaryParams() {
        return crud_util_1.objKeys(this.options.params).filter((param) => this.options.params[param] && this.options.params[param].primary);
    }
    ...
    setRouteArgs(name) {
        ...
        if (crud_util_1.isIn(name, routes)) {
            const action = this.routeNameAction(name);
            const hasDto = !crud_util_1.isNil(this.options.dto[action]);
            const { UPDATE, CREATE } = enums_1.CrudValidationGroups;
            const groupEnum = crud_util_1.isIn(name, ['updateOneBase', 'replaceOneBase']) ? UPDATE : CREATE;
    ...
    setRouteArgsTypes(name) {
        if (crud_util_1.isEqual(name, 'createManyBase')) {
        ...
        else if (crud_util_1.isIn(name, ['createOneBase', 'updateOneBase', 'replaceOneBase'])) {
            const action = this.routeNameAction(name);
    ...
    setInterceptors(name) {
        ...
        reflection_helper_1.R.setInterceptors([
            ...
            ...(crud_util_1.isArrayFull(interceptors) ? interceptors : []),
        ], this.targetProto[name]);
    }
    setDecorators(name) {
        const decorators = this.options.routes[name].decorators;
        reflection_helper_1.R.setDecorators(crud_util_1.isArrayFull(decorators) ? decorators : [], this.targetProto, name);
    }
    setAction(name) {
    ...
    setSwaggerPathParams(name) {
        ...
        const removePrimary = crud_util_1.isIn(name, withoutPrimary);
        const params = crud_util_1.objKeys(this.options.params)

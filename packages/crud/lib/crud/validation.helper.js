...
const validator = util_1.safeRequire('class-validator', () => require('class-validator'));
const transformer = util_1.safeRequire('class-transformer', () => require('class-transformer'));
class BulkDto {
}
class Validation {
    static getValidationPipe(options, group) {
        return validator && !crud_util_1.isFalse(options.validation)
    ...
    static createBulkDto(options) {
        if (validator && transformer && !crud_util_1.isFalse(options.validation)) {
            const { IsArray, ArrayNotEmpty, ValidateNested } = validator;
            const { Type } = transformer;
            const hasDto = !crud_util_1.isNil(options.dto.create);
